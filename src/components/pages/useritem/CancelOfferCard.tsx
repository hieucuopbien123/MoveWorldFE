...
const CancelOfferCard = ({ name, image, creator, collection, fetchData, toAddress }: any) => {
  ...
  const cancelOffer = async () => {
    ...
    try {
      ...
      await MoveServices.cancelOffer({
        data: {
          receiver: toAddress,
          creator,
      ...
      toast(successTopCenter("Cancel offer sucessfully"));
      await fetchData();
      fetchBalance();
      setLoading(false);
    } catch (e: any) {
    ...
    <>
      ...
      >
        ...
        <Button
          ...
          disabled={loading}
        >
          {loading ? <BeatLoader size={8} color="white" /> : "Cancel Offer"}
        </Button>
      </Box>
    </>
  );
...
export default CancelOfferCard;
