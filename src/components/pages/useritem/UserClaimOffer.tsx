...
import getCurentTokensPendingClaim from "@/apis/useritemclaim";

const UserReceivedOffer = ({ ItemTag }: any) => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (account?.address) {
        setLoading(true);
        const response = await getCurentTokensPendingClaim(account?.address);
        setData(
...
export default UserReceivedOffer;
